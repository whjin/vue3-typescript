import { type Node, NodeType, type ResolvedPos, type Mark, type MarkType, type Schema } from "prosemirror-model";
...
export const findNodesWidthSameMark = (doc: Node, from: number, to: number, markType: MarkType) => {
  let ii = from;
...
const findParentNodeCloseToPos = ($pos: ResolvedPos, predicate: (node: Node) => boolean) => {
...
export const findParentNode = (predicate: (node: Node) => boolean) => {
  return (_ref: Selection) => findParentNodeCloseToPos(_ref.$from, predicate);
};
